...
- agent/output/explanations.json — 17 entries (the narrated cases)
...
| Phase 2 confidence | Phase 4 simplified_status | Orders | Settlements | Description |
|---|---|---|---|---|
| matched | Reconciled | 486 | 88 | Exact match, no special handling |
| matched_with_note | Reconciled (with note) | 5 | 0 | Special-case logic (currency conversion, cross-batch refund) |
| hard_exception (NO_CREDIT_EXPECTED) | Reconciled (no credit due) | 0 | 1 | Negative net batch, bank correctly did not credit |
| needs_review | Needs Human Review | 1 | 1 | Requires human judgment |
| hard_exception (other) | Unresolved | 8 | 1 | No match possible (failed payment, NEFT failure, missing settlement) |
...
orders: { total: 500, reconciled: 486, reconciled_with_note: 5,
  needs_human_review: 1, unresolved: 8, match_rate_pct: 98.2 }
settlements: { total: 91, reconciled: 88, reconciled_no_credit_due: 1,
  needs_human_review: 1, unresolved: 1, match_rate_pct: 97.8 }
overall: { total_cases: 591, reconciled_total: 580,
  needs_human_review_total: 2, unresolved_total: 9, match_rate_pct: 98.1 }
...
Every order gets an entry, even the 486 plain-matched ones. Structure varies by status.

**Plain matched (486 entries) — minimal, no explanation:**
...
**Duplicate order (1 entry) — includes conflicting rows:**
  ...
  key_figures: { conflicting_amounts: [1130.56, 1202.36], settlement_ids: [], bank_utr: null }
  soft_flags: []
...
### Step 1: Load inputs
- Read match_log.json (591 entries)
- Read explanations.json (17 entries)
...
4. If not found (575 plain-matched): set all three to null
...
- Compute match_rate_pct = (reconciled + reconciled_with_note) / total * 100
